...
def get_simplified_fresh_id_ranges(
    id_ranges: list[tuple[int, int]]
) -> SortedList[tuple[int, int]]:
    ...
    for range_start, range_end in id_ranges:
        ...
        to_delete = list(
            fresh_id_ranges.irange(range_start, range_end)
        )
        for v in to_delete:
...
def part1():
    ...
    for ingredient_id in ingredient_ids:
        ...
        if (ingredient_id in fresh_id_ranges or
            fresh_id_ranges.bisect_left(ingredient_id) % 2):
            n_fresh += 1
...
def part2():
    ...
    for range_start, range_end in zip(fresh_id_ranges[0::2], fresh_id_ranges[1::2]):
